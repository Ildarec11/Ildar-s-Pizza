...
public class UserServiceImpl implements UserService{
    ...
    public Cookie signIn(SignInUserForm signInUserForm) {
        ...
        if (user != null) {
            if (passwordEncoder.matches(signInUserForm.getPassword(), user.getPassword())) {
                ...
                authRepository.setCookiesForUser(user, cookie);
                return cookie;
            } else {
    ...
    public boolean checkIsAdminByCookieValue(Cookie cookie) {
        return authRepository.checkIsAdminByCookieValue(cookie);
    }

    @Override
    public AuthModel getAuthByCookie(Cookie cookie) {
        return authRepository.findByCookieValue(cookie);
    }

    @Override
    public UserModel getUserByCookie(Cookie cookie) {
        return authRepository.findByCookieValue(cookie).getUserModel();
    }
}
